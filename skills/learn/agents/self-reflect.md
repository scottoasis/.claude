...
The friction ledger at `~/.claude/friction/friction.jsonl` is a JSONL file with one JSON object per line. Each entry has fields: `id`, `date`, `project`, `type`, `description`, `root_cause`, `deep_cause`, `resolution`, `effort`, `constraint_existed`, `constraint_failed`, `recurrence_of`, `prescribed_*`, `implemented_*`, `status`, `domains` (array of tags).

Query with `~/.claude/friction/scripts/query.sh` (commands: domain, failing, chain, recent, stats, search, types, domains, trend, unconstrained, stale, lifecycle) or directly with `jq`.
