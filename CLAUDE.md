...
- **grounding-gate** — Stop: hedge-gate's mirror — blocks the turn when the final prose asserts verification the turn's tool history doesn't back. Class A (evidence-required): `tests pass`, `I ran the tests`, `build succeeds`, `typecheck/lint passes`, `I measured` need a matching successful Bash command THIS TURN (checked against the transcript; failed/interrupted commands don't count). Class B (citation-required): `I checked/verified/confirmed <state>` needs the claiming sentence to say where/how — a path locator, `git -C`, or a backticked command — so a wrong-target check is visible to the reader. Past/perfect claims only (`let me run the tests` never fires), BARE PROSE only, bites ONCE (`stop_hook_active` release; shared with hedge-gate — whichever blocks first spends the response's one reconsideration), fail-open. Every stage-2 evaluation appends to `friction/grounding-gate-ledger.jsonl`. Pre-registered kill criterion (review via `/harness-review`): stage-2 firing on >5% of turns or mostly-false blocks → narrow wordlists or retire, on ledger data. Origin: rage-log FM-1 (2026-06-26 "DID YOU EVEN RAN TEST", 2026-07-07 wrong-directory "I just checked"). Smoke tests: `hooks/test-grounding-gate.sh`. (`hooks/grounding-gate.sh`)
